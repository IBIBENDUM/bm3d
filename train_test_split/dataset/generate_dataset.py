...
def processAndSaveImages(imageFiles, sourceDir, cleanDir,
    ...
    for file in imageFiles:
        ...
        croppedImage = cropImage(image, cropSize)

        cv2.imwrite(os.path.join(cleanDir, file), croppedImage)

        for variance, noisyDir in noisyDirs.items():
            noisyImage = addNoise(croppedImage, variance)
...
def splitDataset(sourceDir: str="original_dataset", 
    ...
    imageFiles = getValidImagePaths(sourceDir) 
